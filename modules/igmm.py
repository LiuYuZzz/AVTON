...
MAX_ALPHA = torch.Tensor([
    8.00**0.5, 6.25**0.5, 5.00**0.5, 6.25**0.5, 8.00**0.5,

    6.25**0.5, 4.50**0.5, 3.25**0.5, 4.50**0.5, 6.25**0.5,

    5.00**0.5, 3.25**0.5, 2.00**0.5, 3.25**0.5, 5.00**0.5,
...
class FeatureRegression(nn.Module):
    def __init__(self, input_nc=192, output_dim=6, use_cuda=True):
        ...
        if use_cuda:
            ...
            self.tanh.cuda()
            self.sigmoid.cuda()
            self.MAX_ALPHA = self.MAX_ALPHA.cuda()

    def forward(self, x):
        x = self.conv(x)
        x = x.contiguous().view(x.size(0), -1)
        x = self.linear(x)
        theta, alpha = torch.split(x, [50, 25], dim=1)
        theta = self.tanh(theta)
        # alpha = self.sigmoid(alpha) * self.MAX_ALPHA
        # alpha = self.sigmoid(alpha) * 1.5 + 0.5 ** 0.5
        alpha = self.sigmoid(alpha) * 8 ** 0.5
        return theta, alpha
...
class WendlandGridGen(nn.Module):
    def __init__(self, out_h=256, out_w=192, use_regular_grid=True, grid_size=3, reg_factor=0, use_cuda=True):
        super(WendlandGridGen, self).__init__()
        ...
        if use_regular_grid:
            ...
            self.P_X_clone = P_X.clone()
            self.P_Y_clone = P_Y.clone()
            if use_cuda:
                P_X = P_X.cuda()
                P_Y = P_Y.cuda()
                self.P_X_base = self.P_X_base.cuda()
                self.P_Y_base = self.P_Y_base.cuda()
                self.P_X_clone = self.P_X_clone.cuda()
                self.P_Y_clone = self.P_Y_clone.cuda()
            self.P_X = P_X.unsqueeze(2).unsqueeze(
                3).unsqueeze(4).transpose(0, 4)
            self.P_Y = P_Y.unsqueeze(2).unsqueeze(
                3).unsqueeze(4).transpose(0, 4)

    def forward(self, theta, alpha):
        warped_grid = self.apply_transformation(
            theta, torch.cat((self.grid_X, self.grid_Y), 3), alpha)

        return warped_grid

    def compute_L_inverse(self, X, Y, alpha, batch_size):
        N = X.size()[0]  # num of points (along dim 0)
        # construct matrix K
        Xmat = X.expand(batch_size, N, N)
        Ymat = Y.expand(batch_size, N, N)
        P_dist_squared = torch.sqrt(torch.pow(
            Xmat - Xmat.transpose(1, 2), 2) + torch.pow(Ymat - Ymat.transpose(1, 2), 2))
        K = torch.mul(torch.pow(F.relu(1 - P_dist_squared / alpha),
                      4), (4 * P_dist_squared / alpha + 1))
        # construct matrix L
        O = torch.FloatTensor(batch_size, N, 1).fill_(1).cuda()
        Z = torch.FloatTensor(batch_size, 3, 3).fill_(0).cuda()
        P = torch.cat((O, X.expand((batch_size, N, 1)),
                      Y.expand((batch_size, N, 1))), 2)
        L = torch.cat((torch.cat((K, P), 2), torch.cat(
            (P.transpose(1, 2), Z), 2)), 1)
        Li = torch.inverse(L)
        if self.use_cuda:
            Li = Li.cuda()
        return Li

    def apply_transformation(self, theta, points, alpha):
        if theta.dim() == 2:
        ...
        Li = self.compute_L_inverse(
            self.P_X_clone, self.P_Y_clone, alpha, batch_size)
        ...
        dist_squared = torch.sqrt(
            torch.pow(delta_X, 2) + torch.pow(delta_Y, 2))

        dist_squared = dist_squared.expand_as(W_X)
        # U: size [1,H,W,1,N]
        alpha = alpha.unsqueeze(1).unsqueeze(2).unsqueeze(3)
        U = torch.mul(torch.pow(F.relu(1 - dist_squared / alpha),
                      4), (4 * dist_squared / alpha + 1))
        ...
        points_X_prime = A_X[:, :, :, :, 0] + \
            ...
            torch.sum(torch.mul(W_X, U), 4)

        points_Y_prime = A_Y[:, :, :, :, 0] + \
            torch.mul(A_Y[:, :, :, :, 1], points_X_batch) + \
            torch.mul(A_Y[:, :, :, :, 2], points_Y_batch) + \
            torch.sum(torch.mul(W_Y, U), 4)

        return torch.cat((points_X_prime, points_Y_prime), 3)


class IGMM(nn.Module):
    def __init__(self, opt):
        super(IGMM, self).__init__()
        # extract the reference person feature
        self.extractionA = FeatureExtraction(
            30, ngf=64, n_layers=3, norm_layer=nn.BatchNorm2d)
        # extract the target clothing feature
        ...
        # predict relative parameters of Wendland
        self.regression = FeatureRegression(
            input_nc=192, output_dim=2 * opt.grid_size ** 2 + opt.grid_size ** 2, use_cuda=True)
        self.gridGen = WendlandGridGen(
    ...
    def forward(self, inputA, inputB):
        ...
        theta, alpha = self.regression(correlation)
        grid = self.gridGen(theta, alpha)
